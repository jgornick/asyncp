...
describe('everySeries', function() {
    it('does delayed items true', function() {
        let order = [];
        const arr = [3, 2, 1];
        const p = async.everySeries(arr, iterateeDelayWithOrder(order, (x) => x >= 1));
    ...
    it('does sync promise items', function() {
        let order = [];
        const arr = [3, 2, 1];
        const p = async.everySeries(arr, iterateePromiseWithOrder(order, (x) => x >= 1));
        return Promise.all([
            p.should.eventually.equal(true),
            p.then(() => order.should.deep.equal(arr))
        ]);
    ...
    it('does sync native items', function() {
        let order = [];
        const arr = [3, 2, 1];
        const p = async.everySeries(arr, iterateeNativeWithOrder(order, (x) => x >= 1));
        return Promise.all([
            p.should.eventually.equal(true),
            p.then(() => order.should.deep.equal(arr))
        ]);
    ...
    it('does mixed (delayed, promise, native) items', function() {
        ...
        const p = async.everySeries(arr, value => {
            if (value >= 1 && value <= 3) {
    ...
    it('has right arguments', function() {
        const arr = [1, 3, 2];
        return async.everySeries(arr, (value, index, collection) => {
            switch (value) {
                case 1:
    ...
    it('supports external array modification with mixed (delayed, sync promise) items', function() {
        let order = [];
        let arr = [4, 3, 2, 1];
        const p = async.everySeries(arr, (value, index, collection) => {
            if (value >= 2 && value <= 3) {
                return new Promise(resolve => {
                    setTimeout(
                        () => {
    ...
    it('rejects using delayed Promise.reject', function() {
        let order = [];
        const arr = [1, 3, 2];
        const p = async.everySeries(arr, iterateeDelayWithOrder(
            order,
    ...
    it('rejects using sync Promise.reject', function() {
        let order = [];
        const arr = [1, 3, 2];
        const p = async.everySeries(arr, iterateePromiseWithOrder(
            order,
    ...
    it('rejects using sync throw', function() {
        let order = [];
        const arr = [1, 3, 2];
        const p = async.everySeries(arr, iterateeNativeWithOrder(
            order,
            (x) => {
                if (x == 3) {
    ...
    it('has allSeries alias', function(done) {
        assert.strictEqual(async.everySeries, async.allSeries);
        done();
    });
});
