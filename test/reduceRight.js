...
describe('reduceRight', function() {
    it('does delayed items', function() {
        let order = [];
        const arr = [1, 3, 2];
        const p = async.reduceRight(arr, 0, accIterateeDelayWithOrder(order, (acc, x) => acc + x));
        return Promise.all([
    ...
    it('does sync promise items', function() {
        let order = [];
        const arr = [1, 3, 2];
        const p = async.reduceRight(arr, 0, accIterateePromiseWithOrder(order, (acc, x) => acc + x));
        return Promise.all([
            p.should.eventually.equal(6),
            p.then(() => order.should.deep.equal(arr.reverse()))
        ]);
    ...
    it('does sync native items', function() {
        let order = [];
        const arr = [1, 3, 2];
        const p = async.reduceRight(arr, 0, accIterateeNativeWithOrder(order, (acc, x) => acc + x));
        return Promise.all([
            p.should.eventually.equal(6),
            p.then(() => order.should.deep.equal(arr.reverse()))
        ]);
    ...
    it('does mixed (delayed, promise, native) items', function() {
        ...
        const p = async.reduceRight(arr, 0, (acc, value) => {
            if (value >= 1 && value <= 3) {
    ...
    it('has right arguments', function() {
        const arr = [1, 3, 2];
        return async.reduceRight(arr, 0, (acc, value, index, collection) => {
            switch (value) {
                case 1:
                    acc.should.equal(5, `acc is invalid for value ${value}`)
                ...
                case 2:
                    ...
                    break;
                case 3:
                    acc.should.equal(2, `acc is invalid for value ${value}`)
    ...
    it('supports empty collections with custom own property', function() {
        ...
        const p = async.reduceRight(arr, 0, () => assert(false, 'iteratee should not be called'));
    ...
    it('supports external array modification with mixed (delayed, sync promise) items', function() {
        let order = [];
        let arr = [4, 3, 2, 1];
        const p = async.reduceRight(arr, 0, (acc, value, index, collection) => {
            if (value >= 2 && value <= 3) {
                return new Promise(resolve => {
                    setTimeout(
                        () => {
    ...
    it('rejects using delayed Promise.reject', function() {
        let order = [];
        const arr = [1, 3, 2];
        const p = async.reduceRight(arr, 0, accIterateeDelayWithOrder(
            order,
        ...
        return Promise.all([
            ...
            new Promise(resolve => setTimeout(
                () => resolve(order.should.deep.equal([2])),
                7 * 25
            ))
        ]);
    });
    ...
    it('rejects using sync Promise.reject', function() {
        let order = [];
        const arr = [1, 3, 2];
        const p = async.reduceRight(arr, 0, accIterateePromiseWithOrder(
            order,
    ...
    it('rejects using sync throw', function() {
        let order = [];
        const arr = [1, 3, 2];
        const p = async.reduceRight(arr, 0, accIterateeNativeWithOrder(
            order,
            (acc, x) => {
                if (x == 3) {
    ...
    it('has foldr alias', function(done) {
        assert.strictEqual(async.reduceRight, async.foldr);
        done();
    });
});
